...
public class EntityPlayerLeft extends Entity {
    ...
    public void update() {
        ...
        if (dir*mult == -1) {
            if(getPosition().x!=-Settings.SCREEN_WIDTH/2+Settings.WALL_WIDTH)
        ...
        if(pos.x<-Settings.SCREEN_WIDTH/2+Settings.WALL_WIDTH)
            pos.x = -Settings.SCREEN_WIDTH/2+Settings.WALL_WIDTH;
        else if(pos.x>-width-Settings.WALL_WIDTH)
            pos.x = -width-Settings.WALL_WIDTH;
    }
